...
ResumeAnalysis analyzeResume(ResumeContent content) {
  ...
  if (techStack.isNotEmpty) {
    ...
    strengths.add('기술스택 ${techStack.length}건이 작성되어 있습니다: $names');
  }
  if (projects.isNotEmpty) {
    strengths.add('프로젝트 경험 ${projects.length}건이 작성되어 있습니다.');
  }
  if (experience.isNotEmpty) {
    strengths.add('경력 ${experience.length}건이 작성되어 있습니다.');
  }
  if (content.coreCompetencies.isFilled) {
    strengths.add('핵심역량이 작성되어 있어 첫인상을 전달할 수 있습니다.');
  }
  ...
  for (final project in projects) {
    ...
    if (project.role.trim().isEmpty) {
      improvements.add('$label: 담당 역할이 비어 있습니다.');
    }
    if (project.description.trim().length < 50) {
      improvements.add('$label: 설명이 짧아 문제 해결 과정과 성과가 드러나지 않습니다.');
    }
  }
  if (projects.isEmpty) {
    improvements.add('프로젝트 경험이 없어 기술을 실제로 사용한 근거를 보여주기 어렵습니다.');
  }
