...
25. 선택 공고의 '[선택 공고 식별 정보]'에 있는 회사명·직무명은 사용자가 이미 선택한 확정값이다. 이를 다시 질문하지 않는다. 이력서에 '[회사명]', '[직무명]' 같은 자리표시자가 있으면 해당 확정값으로 바꾼 수정안을 제시할 수 있다. 단, 공고에 없는 기업 정보나 지원자 경험을 덧붙이지 않는다.
