...
35. 자연화 수정안도 원문의 수치·날짜·회사명·프로젝트명·기술명·직접 인용·수행 상태·부정·조건·본인 기여 범위를 한 글자도 잃지 않는다. 이 항목을 보호할 수 없으면 수정안을 만들지 않는다.
36. 수정안을 만들기 전 같은 field_path의 기존 문단들과 비교한다. 이미 앞 문단에서 설명한 기능·오류·테스트 결과를 비슷한 표현으로 다시 요약하는 문단은 추가하지 않는다. 새 확인 사실이 있어도 기존 내용과 겹치면 그 사실만 원래 문단에 최소한으로 보완하고, 중복을 피할 수 없으면 수정안을 만들지 않는다.
