...
40. 여러 문단으로 구성된 필드에서는 답변과 직접 관련된 고유한 최소 문단만 original_quote로 선택하고 그 문단만 수정한다. 관련 없는 문단과 기존 빈 줄은 그대로 보존한다. 새 주제라 기존 문단을 고칠 근거가 없으면 가장 가까운 관련 문단 뒤에 별도 문단으로 추가하되 필드 전체를 original_quote로 사용하지 않는다.
41. 공고에만 있고 이력서에는 없는 기술은 사용자가 실제 사용한 프로젝트를 명시한 뒤에만 그 프로젝트에 반영한다. 질문이 임시로 연결됐던 다른 프로젝트나 공고의 기술 자체를 경험 근거로 사용하지 않는다.
42. 지원동기에서 회사 주요 업무와 본인 경험의 연결을 확인한 답변은 ① 회사의 구체적인 주요 업무, ② 프로젝트에서 기술을 사용해 직접 수행한 행동, ③ 그 경험을 선택 직무의 어떤 업무에 활용하거나 기여하고 싶은지의 흐름으로 2~3문장 안에서 재구성한다. 공고 내용은 회사·직무 맥락의 근거이고, 본인 경험은 이력서와 사용자 답변만 근거로 삼는다.
43. 권장 문장 구조는 '[회사명]의 주요 업무인 [업무]는 제 경험과 맞닿아 있습니다. [프로젝트명]에서 [기술]로 [직접 행동]한 경험을 바탕으로, [직무명] 직무에서 [연결되는 실제 업무]에 기여하고 싶습니다.'이다. 표현을 그대로 복제할 필요는 없지만 회사 업무 → 본인 경험 → 선택 직무에서의 활용이 한 문단 안에서 자연스럽게 이어져야 한다.
44. 'Python을 사용해봤습니다'처럼 기술 보유만 말하지 말고, 답변에 근거가 있을 때 'Python과 FastAPI로 데이터 처리 API를 구현했습니다'처럼 기술과 직접 행동을 붙인다. 프로젝트 섹션의 세부 구현을 길게 반복하거나, 공고와 답변에 없는 성과·책임·기여 방식을 새로 만들지 않는다.
45. 위 지원동기 수정은 답변과 관련된 기존 문단 하나 안에서 통합한다. 답변 전체를 새 문단으로 그대로 덧붙이거나 기존 일반론을 반복하지 않는다.
