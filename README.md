...
| 김대호 | PM · 역할별 LMS 구축 · Firebase 운영 · LLMOps · AI 복습 도우미 · AI 성취도평가 |
| 문성호 | 채용공고·이력서 기반 RAG 아키텍처 설계, Grounded LLM 첨삭 워크플로우 구현, 공고 요건 구조화·STAR 분석·사용자 답변 기반 재첨삭·환각 검증, AI 코치 UI/UX 및 앱 공통 테마·사용자 매뉴얼 | |
...
### 김기호
먼저 벡터 데이터를 다루는데 어려웠던 점은, “벡터 데이터를 어떤 기준과 카테고리로 나눌 것이냐” 가 관건이었다. 기본적으로 벡터 데이터는 한 인덱스에 어떤 파일 유형이든, 어떤 카테고리의 데이터든 간에 모두 적재해도 문서 검색은 가능하지만, 당연히 문서 검색 시간도 그만큼 늘어나기 때문에 벡터 데이터를 적절한 인덱스, 네임 스페이스, 메타데이터로 구분하는 것이 중요했다. 
또한 협업 과정에서는 이번 프로젝트 때 적극적으로 github의 브랜치 기능을 활용하였는데, 처음에는 익숙하지 않아서 고민하는 시간이 많았다. 하지만 점차 각자 브랜치에서 작업해나가면서 작업→PR→병합 흐름에 적응한 이후에는 협업은 브랜치 기능으로 각자의 작업 공간을 확실히 격리하는 것이 시작이라는 것을 깨닫게 되었다.
### 김대호
PM으로써 프로젝트 진행에 있어서 조율을 하고 관리를 하는게 재밌었고, 실 사용자인 우리가 직접 불편한 점을 고쳐가면서 개선해나갈 수 있어서 원활하게 프로젝트를 마무리할 수 있었다. 매니저님과 강사님에게 직접 인터뷰를 통해 불편한 점을 찾고 더 완성도 높은 프로젝트로 나아갈 수 있었던 거 같아 뿌듯하다.
### 문성호

처음에는 RAG를 적용하면 관련 문서를 잘 검색하는 것만으로 충분하다고 생각했다. 하지만 이력서 첨삭 기능을 구현하면서, 검색 결과가 정확하더라도 LLM이 사용자의 경험과 공고 내용을 잘못 연결하거나 존재하지 않는 성과를 만들어낼 수 있다는 것을 알게 되었다. 반복적인 테스트를 통해 AI 서비스에서는 자연스러운 답변보다 근거를 명확하게 제한하고, 부족한 정보는 사용자에게 다시 확인하며, 생성 결과를 검증하는 과정이 중요하다는 것을 배웠다.

또한 기능이 동작하는 것과 사용자가 편하게 사용하는 것은 별개의 문제였다. 실제 화면에서 질문과 수정안을 확인하는 과정을 반복하면서 안내 문구와 사용자 흐름을 개선했고, 기술적 정확성과 UI/UX가 함께 설계되어야 서비스의 완성도가 높아진다는 것을 체감했다. 여러 브랜치의 기능을 통합하는 과정에서는 내 작업뿐 아니라 팀 전체의 데이터 흐름과 인터페이스를 이해하는 것이 협업에서 중요하다는 점도 배울 수 있었다.
### 최성욱
프로젝트를 진행하며 실제 사용 중 느꼈던 불편함을 직접 찾아 개선하고, 서비스가 점점 더 나아지는 과정을 체감할 수 있어 뿌듯했으며, 맡은 역할을 수행하는 과정에서 좋은 팀원들과 함께 문제를 해결해 나가는 협업의 즐거움도 느낄 수 있었다.
