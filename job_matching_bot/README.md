...
`cover_letter_rag/`가 그 공고 기준으로 이력서를 봐 준다. 두 모듈은 코드가 분리돼 있고,
나중에 한 프로세스로 합친다.
...
3위로 올라온다(`experiments/embedding_only_ranking.py`로 실측). 조건은 규칙으로
...
Functions와 앱이 읽는 생성 파일을 만들며, 앱이 공고를 들고 있지 않기로 한 뒤로는
쓰임이 줄었다.
