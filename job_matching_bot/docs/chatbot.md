...
서버는 대화를 저장하지 않는다. 앱이 **세 가지**를 되돌려 보낸다.
...
정렬은 상세 있는 공고 → 관련도 → 태그를 적게 단 공고 → 최근에 처음 본 공고 순이다. 태그를 열 개씩
단 "전 직군 공개채용"은 무엇을 물어도 걸리므로 그 일에 특화된 공고에 자리를 내준다. 한 번에
3,000행까지 훑고, 넘으면 "N건이 넘어요"라고 답한다.

답의 "N건 중 직무가 맞는 건 M건"에서 M은 관련도 2 이상이다. 본문에 말이 스친 공고까지 세면
실제보다 훨씬 많아 보인다. **질문 갈래의 집계도 같은 조건 함수를 쓴다** — "412건 중 Spring
...
| 프롬프트 | 파일 | 입력 | 출력 |
|---|---|---|---|
| 라우터 `CHAT_PROMPT` | `api/prompts.py` | 말 + 직전 조건 | 주제, 의도, 조건, 가리킨 번호, 직전 답 가리킴, 없는 정보, 뜻 검색 문장, 확인 한 줄 |
...
fixtures/chat_cases.json    평가 케이스 36개, 43턴

tests/test_job_chat.py            94개 — 갈래·막기·맥락·번호·비교·마감·시간
tests/test_job_chat_endpoint.py   15개 — HTTP 경로
tests/test_store_search.py        34개 — 조건·연차·마감
tests/test_search_spelling.py      9개 — 표기 접기
